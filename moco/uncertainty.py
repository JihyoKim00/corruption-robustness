...
parser.add_argument('-moco', action='store_true', dest='pico',
                    help='true if ckpt dir has moco checkpoint')
args = parser.parse_args()

cuda = torch.cuda.is_available()
device = torch.device("cuda" if cuda else "cpu")

def get_values(model, loader, temperature):
    model.eval()
...
def uncertainty(model, loader, type='clean'):
    best_nll = np.inf
    ...
    for temp in temp_list:
        if temp > 0.0:
            dict_values, acc = get_values(model, loader, temp)
    ...
    dict_values, acc = get_values(model, loader, best_temp)
...
for dataset in args.dataset:
    ...
    for q in args.q:
        ...
        if args.moco:
            ...
            load_path = f'{args.ckpt_dir}/{folder}/last-model.pth'
